...
class UserLocationRepository:
    ...
    def get_active_user_location(self, db: Session, user_id: int) -> Optional[int]:
        locations = db.query(UserLocation.location_id).filter(
            UserLocation.user_id == user_id and UserLocation.is_active == True).first()
    ...
    def add_user_location(self, db: Session, user_id: int, location_id: int) -> None:
        db.add(UserLocation(user_id=user_id, location_id=location_id))
        db.commit()
